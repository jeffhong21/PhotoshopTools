...
//Find files inside a folder with path path
    findFiles: function (path) {
        ...
        var sourceFolder = new Folder(path + "/custom/");
        a.writeln("sourceFolder: " + sourceFolder);
        ...
        var fol = new Folder(path + "/preference/");
        if (!fol.exists)
            fol.create();

        if (result.length != 0) {
            var fOrder = new File(path + "/preference/order");
            if (!fOrder.exists) {
                fOrder.open('w');
        ...
        if (result.length != 0) {
            a.writeln("fixarray path : " + path + "/preference/order");
            result = $._ext_PHXS.fixArray(path + "/preference/order", result, false);
        }
        if (resultImages.length != 0) {
            resultImages = $._ext_PHXS.fixArray(path + "/preference/order", resultImages, true);
        }
